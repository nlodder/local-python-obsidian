...
const PLUGINCODE = 'local-py'
const PLUGINNAME = 'Local Python Runner'
...
module.exports = class LocalPythonRunner extends Plugin {
  async onload() {
    ...
    if (!fs.existsSync(pythonPath)) {
      new Notice(
        PLUGINNAME + ": Python VENV not found at " + pythonPath,
        5000,
      );
    }
     ...
     * 
     * use ctx object getSectionInfo() which returns object w/ line numbers and full text
     * of orig markdown file. we are looking for 'local-py' following python PLUGINCODE:
     * ```python local-py . . .```
     */
    this.registerMarkdownCodeBlockProcessor('python', (source, el, ctx) => {
      const section = ctx.getSectionInfo(el);
      if (section) {
        // get line where code block starts
        const lines = section.text.split('\n');
        const firstLine = lines[section.lineStart];

        // check if PLUGINCODE exists on line
        if (firstLine.includes(PLUGINCODE))
    ...
    this.addCommand({
      id: "insert-" + PLUGINCODE + "-skeleton",
      name: "Insert " + PLUGINNAME + " Skeleton",
      editorCallback: async (editor) => {
        const skeleton = await this.getFileContent("skeleton.py");
        editor.replaceSelection(`\`\`\`python ${PLUGINCODE}\n${skeleton}\n\`\`\``);
      },
    });
  }
  ...
  async getFileContent(fileName) {
    try {
      ...
      console.error(`${PLUGINNAME}: Failed to load ${fileName}`, e);
  ...
  async renderPythonBlock(source, el) {
    ...
    button.addEventListener("click", async () => {
      ...
      const status = outputArea.createEl("p", {
        text: "Generating " + PLUGINNAME + " output...",
      });
   ...
   * @param {HTMLElement} parent - container element where iframe is appended
   * @param {string} htmlContent - raw HTML string generated by python script
   */
  renderIframe(parent, htmlContent) {
    ...
    iframe.addEventListener("load", () => {
      ...
      // Detect height from the Plotly object or fallback to 450
      const heightValue = plotlyDiv ? parseInt(plotlyDiv.style.height) : 450;

      // add small buffer (20px) to prevent vertical scrollbars
